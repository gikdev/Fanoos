...
using Fanoos.Common.Application.Authorization;
...
namespace Fanoos.Common.Infrastructure.Authorization;
...
internal sealed class CustomClaimsTransformation(IServiceScopeFactory serviceScopeFactory) : IClaimsTransformation {
    public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal) {
        ...
        if (result.IsFailure) {
            throw new FanoosException(nameof(IPermissionService.GetUserPermissionsAsync), result.Error);
        }
